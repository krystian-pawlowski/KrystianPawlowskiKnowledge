...
- **Workspace package pins and the backend's own Package.resolved.** In the workspace Xcode uses only `SolarsplitWorkspace.xcworkspace/xcshareddata/swiftpm/Package.resolved`, one graph for both projects, and ignores `SolarsplitWeb/Package.resolved`. Xcode keeps the workspace pins as long as they satisfy `Package.swift`, and a `main` branch or a version range stays satisfied by an old revision, so when the backend starts using a newer API of HelvetAI, HelvetBexioPackage or another `main` package the workspace build fails inside that package while `swift build` in SolarsplitWeb passes. Seen on 14.09.2026 in `WhatsAppTransports.swift` and `BexioBillingExecutionProvider.swift`, 28 pins behind, and once before on 20.08.2026, the `.bak` next to the file. The Xcode way is File, Packages, Update to Latest Package Versions, which also moves the packages only the iOS project uses. The precise way is to copy the backend's `state` for the packages present in both files into the workspace file, then File, Packages, Resolve Package Versions. Never edit that file or resolve from a shell while the workspace is open in Xcode: the IDE keeps its package graph in memory and then compiles the new checkouts with the old manifests, which shows up as `Reference to undefined version '1.7.0' for availability macro 'System'` in swift-system. Close the workspace first, or Resolve Package Versions afterwards.
...
- **Android emulator, on the host Mac, not here.** Verified 11.09.2026. This Mac is itself a virtual machine, `sysctl -n machdep.cpu.brand_string` says `Apple M2 (Virtual)`, 8 GB of RAM, and `sysctl kern.hv_support` is 0, so no emulator can run in it and `~/.android/avd` is empty. The emulator runs on the physical Mac that hosts this VM, reached as the ssh alias `android-host`, which is 192.168.64.1, the VM's default gateway (`netstat -rn -f inet | grep default`). A tunnel started outside Claude Code, `ssh -fNT -L 127.0.0.1:5037:127.0.0.1:5037 android-host`, forwards the host's adb server into this VM, so no adb server runs here and every `adb` command talks to the host's one: `adb devices` shows `emulator-5554`, AVD `Pixel_10a` on the API 37.1 Google Play arm64 16k image with 4 GB of guest RAM. `adb shell`, `adb install`, `adb logcat` and `adb exec-out screencap` all work from Claude Code shells, sandbox included. `adb emu` does not, the console port 5554 is not forwarded. The emulator's own logs and the macOS crash reports of its process are on the host, this VM never sees them. A second tunnel of the same shape, alias `minio-s3-host`, forwards ports 9000 and 9001 for the local backend's S3 store, next bullet. Check both with `pgrep -fl "ssh -fNT"`.
- **Local S3 for the backend, on the host Mac as well.** Established 15.09.2026. `SolarsplitWeb/.env.development` points the Bucketeer client at `BUCKETEER_AWS_ENDPOINT=http://localhost:9000`, bucket `solarsplits3`, a MinIO served by the host and reached through the `minio-s3-host` tunnel, port 9000 for the S3 API and 9001 for the MinIO console. Nothing S3-related is installed in this VM, no Docker, no MinIO, no LocalStack, so the LocalStack workflow of `solarsplit-architecture` does not apply here. When the tunnel is down, every backend endpoint that reads or writes a file answers 500 after about twenty seconds of retries, the backend log shows `[HelvetBucketeerServiceImplementation] Error during file download` then `POSIXErrorCode(rawValue: 61): Connection refused`, and the web client shows "Could not load this project" with that reason. A missing object would be a 404 from MinIO instead. Check with `curl -s -o /dev/null -w '%{http_code}' http://localhost:9000/minio/health/live`, 200 means up, 000 means the tunnel is down. Restart it outside Claude Code, same shape as the adb one: `ssh -fNT -L 127.0.0.1:9000:127.0.0.1:9000 -L 127.0.0.1:9001:127.0.0.1:9001 minio-s3-host`, reconstructed from the shape observed on 11.09.2026 and to confirm the first time it is used. If the health check still answers 000 afterwards, MinIO itself is stopped on the host.
...
| How the two ssh tunnels, `android-host` and `minio-s3-host`, are started and whether they survive a reboot. Checked 11.09.2026: no launch agent and no shell startup file mentions them, both processes are daemonized under PID 1 and were started within one second of each other, which points to a script run by hand. Both were found down on 15.09.2026, so count on restarting them by hand at the start of any session that needs adb or the local backend's files | `pgrep -fl "ssh -fNT"`, `curl -s -o /dev/null -w '%{http_code}' http://localhost:9000/minio/health/live`, `ls ~/Library/LaunchAgents`, `grep -n android-host ~/.zshrc ~/.zprofile` |
