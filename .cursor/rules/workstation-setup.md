...
- **Workspace package pins drift behind the backend.** The workspace keeps its own `SolarsplitWorkspace.xcworkspace/xcshareddata/swiftpm/Package.resolved`, one graph for both projects, and Xcode never re-syncs it when SolarsplitWeb bumps a pin, since a `main` branch or a version range stays satisfied by the old revision. The backend then fails inside package APIs, `WhatsAppTransports.swift` on HelvetAI and `BexioBillingExecutionProvider.swift` on HelvetBexioPackage on 14.09.2026, 28 pins behind that day. `python3 ~/solarsplit-dev/code/KrystianPawlowskiKnowledge/scripts/align-workspace-pins.py` copies the backend's revision for every package present in both files, keeps a dated `.bak` next to the workspace file and prints the changes, `--check` only reports, `--dry-run` prints without writing. The `post-merge`, `post-checkout` and `post-rewrite` hooks of the SolarsplitWeb clone run it, so a pull or a branch switch realigns the workspace before Xcode builds again. They live in `SolarsplitWeb/.git/hooks/`, outside git, three lines that call the script when it exists, reinstall them if the clone is recreated. The iOS project inside the workspace then resolves the shared packages at the backend's revisions while `SOLARSPLITiOS.xcodeproj` keeps its own pins, so build iOS from its own project when that matters. Compare the two resolved files before touching code whenever a workspace build fails inside a package.
